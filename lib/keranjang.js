...
const daftarSewa = () => {
  ...
  if (keranjang.length === 0) {
    ...
    keranjang.forEach((item, index) => {
      ...
      console.log(`    Durasi: ${item.days} hari`);
...
const checkout = async () => {
  ...
  while (loop) {
    process.stdout.write("\x1Bc");
    const inputCheckout = await input(" Ingin melakukan checkout semua pesanan (y/n)? ");
    if (inputCheckout === "y") {
      history.push(await createInvoice(keranjang));
      keranjang = [];
      await input(" ✅ Kendaraan berhasil di sewa, dan sudah bisa diambil, invoice bisa dilihat di history");
      loop = false;
      continue;
