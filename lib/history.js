...
const inputNama = async () => {
  ...
  while (loop) {
    ...
    namaPenyewa = (await input("Masukkan nama anda (sesuai dengan yang ada di KTP): ")).toUpperCase();
    if (namaPenyewa === "") {
      await input(" ⚠️ Nama tidak boleh kosong ");
    } else {
      loop = false;
      continue;
    }
...
export const createInvoice = async (itemKeranjang = []) => {
  ...
  const namaPenyewa = await inputNama();
...
export const lihatHistory = async (history = []) => {
  let loop = true;
  while (loop) {
    process.stdout.write("\x1Bc");
    ...
    if (history.length === 0) {
      ...
      history.forEach((item) => {
        ...
        console.log(` NAMA PENYEWA: ${item.name} `);
