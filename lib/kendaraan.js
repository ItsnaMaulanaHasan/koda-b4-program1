...
export const pilihKendaraan = async (daftarKendaraan = []) => {
  let loop = true;
  while (loop) {
    ...
    noInput = parseInt(noInput) - 1;
    day = parseInt(day);
    if (noInput >= 0 && noInput < daftarKendaraan.length && day > 0 && !daftarKendaraan[noInput].booked) {
      ...
      if (sameItem >= 0) {
        ...
        await input(`\n ✅ ${dataKendaraanDipilih.nama} berhasil diperbarui dengan durasi sewa ${dataKendaraanDipilih.days} hari`);
        continue;
      } else {
